...
router.post('/table',async function(req,res){
    ...
    const sql = " select tmp.* " +
                " from ( " +
                        ...
                        " union all " +
                        " Select 1 as rowNum ,itemdm.codem,itemDm.PriceOffer,itemDm.ItemCode,itemdm.Name,itemDm.Barcode,itemdm.DePartName, itemdm.Pack,cast(CONVERT(VARCHAR, CAST(a.p1 AS MONEY), 1) AS VARCHAR) as minPrice , cast(CONVERT(VARCHAR, CAST(a.p2 AS MONEY), 1) AS VARCHAR) as  maxPrice,itemdm.TyItemDm,cast(CONVERT(VARCHAR, CAST(ISNULL(c.QTY,'0.00') AS MONEY), 1) AS VARCHAR) as Reserve,cast(CONVERT(VARCHAR, CAST(ISNULL((b.BAL-ISNULL(QTY,0)),'0.00') AS MONEY), 1) AS VARCHAR) as QBal ,cast(CONVERT(VARCHAR, CAST(ISNULL((b.BAL-ISNULL(QTY,0)),'0.00') AS MONEY), 1) AS VARCHAR)  as BAL,  " +
                                                        ...
                                                        " left join ( select itemCode,sum(QTY) as QTY from ReserveProduct  group by itemCode ) c on c.itemCode = a.ItemCode " +    
